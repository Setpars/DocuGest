    ...
    :align="dropdownAlign" :side="dropdownSide" :items="[
      [
        ...(appSettingsStore.settings.app.home.enable
          ...
          : []),
        { label: 'Réglages du compte utilisateur', icon: 'i-mdi:account', handle: () => profileModal.open() },
      ],
      [
        ...(appSettingsStore.mode === 'pc'
          ? [{ label: 'Raccourci clavier', icon: 'i-mdi:keyboard', handle: () => eventBus.emit('global-hotkeys-intro-toggle') }]
          : []),
      ],
      [
        {
