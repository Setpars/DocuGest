...
const form = useForm({
  validationSchema: toTypedSchema(z.object({
    account: z.string().min(1, 'Veuillez entrer un nom d’utilisateur'),
    password: z.string().min(1, 'Veuillez entrer le mot de passe'),
    remember: z.boolean(),
  })),
  initialValues: {
...
<template>
  <div class="p-12 flex-col-stretch-center min-h-500px w-full">
    ...
    <div class="mb-4">
      <FaTabs
        v-model="type" :list="[
          { label: 'Connexion avec identifiant et mot de passe', value: 'default' },
        ]" class="inline-flex"
      />
    </div>
      ...
      <form @submit="onSubmit">
        <FormField v-slot="{ componentField, errors }" name="account">
          <FormItem class="pb-6 relative space-y-0">
            <FormControl>
              <FaInput type="text" placeholder="utilisateur" class="w-full" :class="{ 'border-destructive': errors.length }" v-bind="componentField">
                <template #start>
