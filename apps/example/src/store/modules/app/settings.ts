...
export const useAppSettingsStore = defineStore(
  ...
  () => {
    ...
    applyThemePreference(settings.value.theme, readThemePreference())

    watch(
      () => settings.value.theme,
      (theme) => writeThemePreference(theme),
      { deep: true },
    )
    ...
    function updateSettings(data: SettingsOptions, fromBase = false) {
      const themeBefore = cloneDeep(settings.value.theme)
      ...
      if (fromBase) {
        applyThemePreference(settings.value.theme, themeBefore)
        writeThemePreference(settings.value.theme)
      }
    }
