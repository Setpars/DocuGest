...
export const useAppMenuStore = defineStore(
  ...
  () => {
    ...
    function resolveRouteTitle(route: RouteRecordRaw, parentTitle?: string) {
    ...
    function convertRouteToMenuRecursive(
      ...
      routes.forEach((item) => {
        ...
        const resolvedTitle = resolveRouteTitle(item, parentTitle)
        ...
        const visibleChildren = item.children?.filter(child => child.meta?.menu !== false) ?? []
        if (visibleChildren.length > 0) {
          menuItem.children = convertRouteToMenuRecursive(
            visibleChildren,
            menuItem.path,
            typeof resolvedTitle === 'string' ? resolvedTitle : parentTitle,
          )
        }
