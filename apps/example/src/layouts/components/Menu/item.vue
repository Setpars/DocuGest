...
const itemMeta = computed(() => props.item.meta as MenuRouteMeta | undefined)

const isComingSoon = computed(() => Boolean(itemMeta.value?.comingSoon))

const comingSoonHint = computed(() =>
  String(itemMeta.value?.comingSoonHint ?? 'Fonctionnalité à venir'),
)
...
<template>
  ...
  >
    <router-link v-slot="{ href, navigate }" custom :to="uniqueKey.at(-1) ?? ''">
      <FaTooltip
        :disabled="(!isComingSoon && !!generateTitle(item.meta?.title)) && (!rootMenu.isMenuPopup || level !== 0 || subMenu)"
        :text="isComingSoon ? comingSoonHint : generateTitle(item.meta?.title)"
        :side="rootMenu.props.mode === 'vertical' ? 'right' : 'bottom'"
        class="h-full w-full"
      >
        <Component
          :is="subMenu ? 'div' : (isComingSoon ? 'span' : 'a')" v-bind="{
            ...
            'px-4 py-3': true,
            'cursor-pointer text-[var(--g-sub-sidebar-menu-color)] transition-colors hover-(bg-[var(--g-sub-sidebar-menu-hover-bg)] text-[var(--g-sub-sidebar-menu-hover-color)])': !isComingSoon,
            'cursor-not-allowed opacity-50 text-[var(--g-sub-sidebar-menu-color)]': isComingSoon,
            'text-[var(--g-sub-sidebar-menu-active-color)]! bg-[var(--g-sub-sidebar-menu-active-bg)]!': isItemActive && !isComingSoon,
            'px-3': rootMenu.isMenuPopup && level === 0,
            'py-3': rootMenu.isMenuPopup && level !== 0,
          })" :title="isComingSoon ? comingSoonHint : generateTitle(item.meta?.title)" v-on="{
            ...(!subMenu && !isComingSoon && {
              click: navigate,
            }),
          }"
        >
          ...
          >
            ...
            >
              ...
              <span
                v-if="isComingSoon"
                class="ms-1 inline-flex rounded bg-slate-200/80 px-1 py-px text-[9px] font-semibold text-slate-600 uppercase dark:bg-slate-600/80 dark:text-slate-300"
              >
                À venir
              </span>
            </span>
