...
function tabbarScrollTip() {
  if (tabContainerRef.value?.$el.clientWidth > (tabsRef.value?.ref?.$el.clientWidth ?? 0) && !localStorage.has('tabbarScrollTip')) {
    ...
    const tips = faToast.info('Astuce', {
      description: 'Trop d’onglets pour la zone visible : placez la souris sur la barre d’onglets et utilisez la molette pour faire défiler.',
      position: 'top-center',
      duration: Infinity,
      action: {
        label: 'Compris',
